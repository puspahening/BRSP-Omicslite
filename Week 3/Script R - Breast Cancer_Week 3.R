...
#Dataset: GSE42568 (Payudara Cancer vs Normal)
...
gset <- getGEO("GSE42568", GSEMatrix = TRUE, AnnotGPL = TRUE)[[1]]
...
legend("topright", legend = levels(gset$group),
       fill = unique(group_colors), cex = 0.8)
...
write.csv(topTableResults, "Hasil_GSE42568_DEG.csv")
...
barplot(go_bp, showCategory = 20,
        title = "GO Biological Process – Breast Cancer (GSE42568)")
...
barplot(go_mf, showCategory = 20,
        title = "GO Molecular Function – Breast Cancer (GSE42568)")
...
barplot(kegg_res, showCategory = 20,
        title = "KEGG Pathway – Breast Cancer (GSE42568)")
